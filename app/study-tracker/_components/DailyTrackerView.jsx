...
export default function DailyTrackerView({ state, updateState, showToast }) {
  ...
  const exportTasks = () => {
    const payload = {
      version: 1, type: "qa-tracker-tasks", exportedAt: new Date().toISOString(),
  ...
  return (
    <div>
      ...
      <div className="st-dash-header">
        <div className="st-dash-header-row">
          <div>
            ...
            <p className="st-dash-subtitle">Daily tasks &amp; recurring habits with time tracking</p>
          </div>
          <div className="st-dash-actions">
            <button className="st-action-btn st-export-btn" onClick={exportTasks}>⬇ Export Tasks</button>
            <button className="st-action-btn st-import-btn" onClick={importTasks}>⬆ Import Tasks</button>
          </div>
        </div>
      </div>
      ...
      <div className="st-view-tabs">
        <button className={`st-view-tab${view === "tasks" ? " active" : ""}`} onClick={() => setView("tasks")}>
          📋 Daily Tasks
        </button>
        <button className={`st-view-tab${view === "habits" ? " active" : ""}`} onClick={() => setView("habits")}>
          🔄 Recurring Habits
        </button>
      </div>
          ...
          <div>
            ...
            <div className="st-task-panel">
              ...
              <div className="st-task-list">
                ...
                {habitsForSel.length > 0 && (
                  <>
                    <div className="st-task-section-hdr st-habit-section-hdr">
                      🔄 Recurring Habits
                      <span className="st-task-section-cnt">{habitDone}/{habitsForSel.length}</span>
                    </div>
                    {habitsForSel.map((h) => {
                      const done = !!state.habitLog[selectedDate]?.[h.id];
                      return (
                        <div key={h.id} className={`st-task-item st-habit-item${done ? " done" : ""}`}>
                          <div className={`st-task-check st-habit-check${done ? " done" : ""}`} onClick={() => toggleHabit(h.id)}>
                            {done && CHECK_SVG}
                          </div>
                          <span className={`st-task-text${done ? " done" : ""}`}>{h.title}</span>
                          {h.timeMin > 0 && <span className="st-task-time-badge">{formatMinutes(h.timeMin)}</span>}
                          <span className="st-habit-recur-badge">{formatHabitRecurrence(h)}</span>
                        </div>
                      );
                    })}
                  </>
                ...
                {(tasksForDate.length > 0 || habitsForSel.length > 0) && (
                  <div className="st-task-section-hdr">
                    📋 Daily Tasks
                    <span className="st-task-section-cnt">{taskDone}/{tasksForDate.length}</span>
                  </div>
                )}
                ...
                {tasksForDate.map((task, i) => (
                  <div key={task.id || i} className={`st-task-item${task.done ? " done" : ""}`}>
                    <div className={`st-task-check${task.done ? " done" : ""}`} onClick={() => toggleTask(i)}>
                      {task.done && CHECK_SVG}
                    </div>
                    <span className={`st-task-text${task.done ? " done" : ""}`}>{task.title}</span>
                    {task.timeMin > 0 && <span className="st-task-time-badge">{formatMinutes(task.timeMin)}</span>}
                    <button className="st-task-del-btn" onClick={() => deleteTask(i)}>×</button>
                  </div>
                ))}
              </div>
              <div className="st-add-task-form">
                ...
                />
                <button className="st-add-task-btn" onClick={addTask}>+ Add</button>
              </div>
            </div>
          </div>
  ...
  function renderCharts() {
    ...
    if (progressRef.current) {
      charts.current.progress = new C(progressRef.current, {
        ...
        options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { min: 0, max: 100, ticks: { callback: (v) => v + "%", font: { family: "'DM Sans'" } }, grid: { color: "#f3f4f6" } }, x: { ticks: { font: { family: "'DM Sans'", size: 11 } }, grid: { display: false } } } },
      });
    }
    ...
    if (timePieRef.current) {
      charts.current.timePie = new C(timePieRef.current, {
        ...
        options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: "bottom", labels: { font: { family: "'DM Sans'", size: 12 } } } } },
      });
    }
    ...
    if (barRef.current) {
      charts.current.bar = new C(barRef.current, {
        ...
        options: { responsive: true, plugins: { legend: { position: "bottom", labels: { font: { family: "'DM Sans'", size: 12 } } } }, scales: { x: { stacked: true, ticks: { font: { family: "'DM Sans'", size: 11 } }, grid: { display: false } }, y: { stacked: true, beginAtZero: true, ticks: { stepSize: 1, font: { family: "'DM Sans'" } }, grid: { color: "#f3f4f6" } } } },
      });
    }
    ...
    <div className="st-daily-right">
      <div className="st-analytics-top-row">
        ...
        <div className="st-filter-tabs">
          <button className={`st-filter-tab${filterMode === "weekly" ? " active" : ""}`} onClick={() => setFilterMode("weekly")}>Weekly</button>
          <button className={`st-filter-tab${filterMode === "monthly" ? " active" : ""}`} onClick={() => setFilterMode("monthly")}>Monthly</button>
        </div>
      </div>
      <div className="st-analytics-cards">
        <div className="st-ana-card"><div className="st-ana-val">{cardDone}</div><div className="st-ana-lbl">Items Done</div></div>
        <div className="st-ana-card"><div className="st-ana-val">{cardTotal}</div><div className="st-ana-lbl">Total Items</div></div>
        <div className="st-ana-card"><div className="st-ana-val">{cardPct}%</div><div className="st-ana-lbl">Completion</div></div>
        <div className="st-ana-card"><div className="st-ana-val">{formatMinutes(cardTime)}</div><div className="st-ana-lbl">Time Done</div></div>
      </div>
      <div className="st-chart-card" style={{ marginBottom: 10 }}>
        <h3 className="st-chart-title">
          Completion Rate <span className="st-chart-subtitle">({filterMode === "weekly" ? "last 7 days" : "last 30 days"})</span>
        </h3>
      ...
      <div className="st-charts-row-2">
        <div className="st-chart-card">
          ...
          <div className="st-pie-wrap"><canvas ref={timePieRef} /></div>
        </div>
        <div className="st-chart-card">
...
function HabitsView({ state, habitForm, setHabitForm, onAddHabit, onDeleteHabit }) {
  ...
  useEffect(() => {
    const load = () => { if (window.Chart) { renderHabitCharts(); return; } const s = document.createElement("script"); s.src = "https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"; s.onload = renderHabitCharts; document.head.appendChild(s); };
    load();
    return () => { Object.values(charts.current).forEach((c) => { try { c.destroy(); } catch (_) {} }); charts.current = {}; };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function renderHabitCharts() {
    ...
    if (barRef.current && state.habits.length) {
      ...
      charts.current.bar = new C(barRef.current, { type: "bar", data: { labels, datasets: [{ label: "Completion %", data: rates, backgroundColor: colors, borderRadius: 5 }] }, options: { indexAxis: "y", responsive: true, plugins: { legend: { display: false } }, scales: { x: { min: 0, max: 100, ticks: { callback: (v) => v + "%", font: { family: "'DM Sans'" } }, grid: { color: "#f3f4f6" } }, y: { ticks: { font: { family: "'DM Sans'", size: 11 } }, grid: { display: false } } } } });
    }

    // This week
    if (weekRef.current) {
      const wlabels = [], wDone = [], wSched = [];
      for (let i = 6; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); const str = d.toISOString().slice(0, 10); wlabels.push(d.toLocaleDateString("en-US", { weekday: "short" })); const habitsDay = state.habits.filter((h) => habitAppliesOnDate(h, str)); wSched.push(habitsDay.length); wDone.push(habitsDay.filter((h) => state.habitLog[str]?.[h.id]).length); }
      charts.current.week = new C(weekRef.current, { type: "bar", data: { labels: wlabels, datasets: [{ label: "Done", data: wDone, backgroundColor: "#10b981", borderRadius: 4 }, { label: "Scheduled", data: wSched.map((s, i) => s - wDone[i]), backgroundColor: "#e5e7eb", borderRadius: 4 }] }, options: { responsive: true, plugins: { legend: { position: "bottom", labels: { font: { family: "'DM Sans'", size: 12 } } } }, scales: { x: { stacked: true, ticks: { font: { family: "'DM Sans'", size: 11 } }, grid: { display: false } }, y: { stacked: true, beginAtZero: true, ticks: { stepSize: 1, font: { family: "'DM Sans'" } }, grid: { color: "#f3f4f6" } } } } });
    }
  }
    ...
    <div>
      <div className="st-habits-layout">
        ...
        <div className="st-habit-form-card">
          <h3 className="st-chart-title" style={{ marginBottom: 14 }}>➕ New Recurring Habit</h3>
          <div className="st-hf-row">
            <div className="st-hf-group" style={{ flex: 2 }}>
              <label className="st-hf-label">Habit Name</label>
              <input className="st-hf-input" placeholder="e.g. Morning Review" value={habitForm.title} onChange={(e) => setHabitForm((f) => ({ ...f, title: e.target.value }))} />
            </div>
            <div className="st-hf-group">
              <label className="st-hf-label">Time (min)</label>
              <input type="number" className="st-hf-input" placeholder="30" min={1} value={habitForm.time} onChange={(e) => setHabitForm((f) => ({ ...f, time: e.target.value }))} />
            </div>
          </div>
          <div className="st-hf-group">
            ...
            <div className="st-hf-pills">
              {["daily","weekdays","weekends","custom"].map((r) => (
                <button key={r} className={`st-pill${habitForm.recurrence === r ? " active" : ""}`} onClick={() => setHabitForm((f) => ({ ...f, recurrence: r }))}>
          ...
          {habitForm.recurrence === "custom" && (
            <div className="st-hf-group">
              ...
              <div className="st-day-toggles">
                {DAYS.map((d, i) => (
                  <button key={i} className={`st-day-toggle${habitForm.customDays.includes(i) ? " active" : ""}`} onClick={() => setHabitForm((f) => ({ ...f, customDays: f.customDays.includes(i) ? f.customDays.filter((x) => x !== i) : [...f.customDays, i] }))}>
                    {d}
                  </button>
                ))}
              </div>
          ...
          <div className="st-hf-group">
            ...
            <div className="st-hf-pills">
              {[["1week","1 Week"],["2weeks","2 Weeks"],["3weeks","3 Weeks"],["1month","1 Month"],["2months","2 Months"],["indefinite","Indefinite"],["custom","Custom"]].map(([v,l]) => (
                <button key={v} className={`st-pill${habitForm.duration === v ? " active" : ""}`} onClick={() => setHabitForm((f) => ({ ...f, duration: v }))}>{l}</button>
              ))}
            </div>
          </div>
          <div className="st-hf-row">
            <div className="st-hf-group">
              ...
              <input type="date" className="st-hf-input" value={habitForm.startDate} onChange={(e) => setHabitForm((f) => ({ ...f, startDate: e.target.value }))} />
            </div>
            <div className="st-hf-group">
              <label className="st-hf-label">
                End Date {computedEnd && <span className="st-hf-computed">(→ {computedEnd})</span>}
              </label>
              <input type="date" className="st-hf-input" value={computedEnd || habitForm.endDate || ""} disabled={habitForm.duration !== "indefinite" && habitForm.duration !== "custom"} onChange={(e) => setHabitForm((f) => ({ ...f, endDate: e.target.value }))} />
            </div>
          </div>
          <button className="st-add-habit-btn" onClick={onAddHabit}>Create Habit</button>
        </div>

        {/* Habits list */}
        <div className="st-habits-list-card">
          <h3 className="st-chart-title" style={{ marginBottom: 12 }}>Your Habits</h3>
          {state.habits.length === 0
            ? <div className="st-no-tasks" style={{ padding: 18 }}>No habits yet. Create one using the form!</div>
                ...
                <div key={h.id} className="st-habit-list-item">
                  ...
                  <div className="st-habit-list-body">
                    ...
                    <div className="st-habit-list-meta">
                      {formatHabitRecurrence(h)} {h.timeMin ? "· " + formatMinutes(h.timeMin) : ""} · {h.startDate} {h.endDate ? "→ " + h.endDate : "∞"} · <span style={{ color: "#10b981", fontWeight: 600 }}>{done}/{sched} ({rate}%)</span>
                    </div>
                    <div className="st-habit-rate-bar"><div className="st-habit-rate-fill" style={{ width: rate + "%" }} /></div>
                  </div>
                  <button className="st-task-del-btn" style={{ fontSize: "1rem" }} onClick={() => onDeleteHabit(i)}>×</button>
                </div>
              );
            })
          }
      ...
      <div className="st-habit-analytics-row">
        <div className="st-chart-card"><h3 className="st-chart-title">Habit Completion Rates</h3><canvas ref={barRef} height={160} /></div>
        <div className="st-chart-card"><h3 className="st-chart-title">Habits This Week</h3><canvas ref={weekRef} height={160} /></div>
      </div>
    </div>
  );
}

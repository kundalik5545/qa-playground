    ...
    (topicId, checked, topic) => {
      ...
      if (topic.subtopics?.length) {
        ...
        topic.subtopics.forEach((_, i) => {
          newSubtopics[topicId][i] = checked;
        });
      }
      ...
      if (checked) {
        ...
        if (last && last.date === today && last.tabId === syllabus.id)
          last.count += 1;
  ...
  const handleNoteSave = useCallback(
    (topicId, value) => {
      const newCustom = {
        ...state.custom,
        [topicId]: {
          ...(state.custom[topicId] || { resources: [] }),
          notes: value,
        },
      };
    ...
    (topicId, label, url) => {
      ...
      const newCustom = {
        ...
        [topicId]: {
          ...
          resources: [
            ...existing.resources,
            { label: label.trim() || url.trim(), url: url.trim() },
          ],
        },
      };
      updateState("custom", newCustom);
  ...
  const handleDeleteResource = useCallback(
    (topicId, idx) => {
      ...
      const newCustom = {
        ...
        [topicId]: {
          ...existing,
          resources: existing.resources.filter((_, i) => i !== idx),
        },
      };
      updateState("custom", newCustom);
    },
      ...
      >
        <div className="st-syl-title-row">
          ...
          <div className="st-syl-ring-wrap">
            ...
            <span className="st-ring-pct" style={{ color }}>
              {pct}%
            </span>
          </div>
        </div>
            ...
            <div className="st-section-card" key={sec.id}>
              ...
              >
                <div className="st-sec-title-wrap">
                  <span className={`st-sec-chevron${isOpen ? " open" : ""}`}>
                    ▶
                  </span>
                ...
                <div className="st-section-body">
                  {sec.topics.map((topic) => (
                    <TopicCard
                      ...
                      customData={
                        custom[topic.id] || { notes: "", resources: [] }
                      }
                      subtopicState={subtopics[topic.id] || {}}
                      onToggle={() => toggleTopic(topic.id)}
                      onCheck={(checked) =>
                        handleCheck(topic.id, checked, topic)
                      }
                      ...
                      onAddResource={(label, url) =>
                        handleAddResource(topic.id, label, url)
                      }
                      onDeleteResource={(idx) =>
                        handleDeleteResource(topic.id, idx)
                      }
                    />
                  ))}
                </div>
...
function TopicCard({
  ...
  return (
    <div
      className={`st-topic-card${isDone ? " done" : ""}`}
              ...
              <div className="st-subtopics-checklist">
                {topic.subtopics.map((st, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`st-subtopic-check${checked ? " checked" : ""}`}
                        style={{ "--syl-color": color }}
                      >
                        {checked && SMALL_CHECK_SVG}
                      </div>
                      <span
                        className={`st-subtopic-label${checked ? " done" : ""}`}
                      >
                        {st}
                      </span>
                    </div>

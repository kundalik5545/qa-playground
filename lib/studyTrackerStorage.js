...
import { getIdbItem, setIdbItem, clearDb } from "@/lib/indexedDb";
...
export const STORAGE_KEYS = {
  ...
  order: "qa_tracker_syllabus_order",
};
...
export async function loadStateFromIdb() {
  ...
  try {
    const data = await getIdbItem("study-tracker-state");
    return data;
  } catch (e) {
...
export async function saveStateToIdb(state) {
  ...
  try {
    await setIdbItem("study-tracker-state", state);
  } catch (e) {
...
export function loadAllState() {
  ...
  try {
    ...
    const order = JSON.parse(
      localStorage.getItem(STORAGE_KEYS.order) || "null",
    );
    const syllabusOrder = Array.isArray(order) ? order : Object.keys(syllabi);
    ...
    return {
      ...
      subtopics: JSON.parse(
        localStorage.getItem(STORAGE_KEYS.subtopics) || "{}",
      ),
...
export async function saveKey(key, value) {
  ...
  const existing = await getIdbItem("study-tracker-state");
  const next = existing ? { ...existing, [key]: value } : { [key]: value };
  await setIdbItem("study-tracker-state", next);
}
...
export function getSyllabusStats(syllabi, progress = {}, syllabusId) {
  ...
  for (const sec of syl.sections) {
    if (!sec || !Array.isArray(sec.topics)) continue;
...
export function getAllStats(syllabi, progress) {
  if (!syllabi) return { total: 0, done: 0, pct: 0 };
  let total = 0,
    done = 0;
